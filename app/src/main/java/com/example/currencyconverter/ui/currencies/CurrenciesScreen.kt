    ...
    Column(modifier = Modifier.padding(16.dp)) {
        Row(modifier = Modifier.fillMaxWidth()) {
            OutlinedTextField(
                ...
                onValueChange = {
                    amount = it.toDoubleOrNull() ?: 1.0
                },
                modifier = Modifier.weight(1f),
                keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number)
            )
            Button(onClick = onHistory, modifier = Modifier.padding(start = 8.dp)) {
                Text("History")
            }
        }
        ...
        rates.forEach { rate ->
            Button(
                onClick = { selected = rate.currency; onSelect(rate.currency, amount, rate.value) },
                modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp)
            ) {
                Text(text = "${rate.currency.name}: ${rate.value}")
            }
        }
    }
}
